...
function startsWith(x, str) {
  return str.lastIndexOf(x, 0) === 0;
}

var UNCAMELIZE_REGEX = /[A-Z]/g;
function uncamelize(str) {
  return str.replace(UNCAMELIZE_REGEX, "-$&").toLowerCase();
}

function applyStyles(dom, attrs, prefix) {
  _.each(attrs, function(v, k) {
    if (startsWith(prefix, k)) {
      k = k.charAt(prefix.length).toLowerCase() +
        uncamelize(k.slice(prefix.length + 1));
      dom.style(k, v);
    }
  });
}
