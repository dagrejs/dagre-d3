...
function defaultDrawNodes(g, root) {
  var nodes = g.nodes().filter(function(u) { return !isSubgraph(g, u); });
...
function addLabel(node, root, addingNode, marginX, marginY) {
  ...
  if (typeof label === "string") {
    if (label[0] === "<") {
      ...
      innerLabelSvg = addTextLabel(label, labelSvg);
...
function addForeignObject(modifier, root) {
  var fo = root
...
function addForeignObjectLabel(label, root) {
  addForeignObject(function(e) {
    e.html(function() { return label; });
  }, root);
}

function addForeignObjectElementFunction(elemFunc, root) {
  addForeignObject(function(e) {
    e.insert(elemFunc);
  }, root);
}

function addForeignObjectElement(elem, root) {
  addForeignObjectElementFunction(function() { return elem; }, root);
}
...
function isSubgraph(g, u) {
  return g.hasOwnProperty("children") && g.children(u).length;
}
