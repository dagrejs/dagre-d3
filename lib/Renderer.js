...
function Renderer() {
  ...
  this._edgeInterpolate = 'bundle';
  this._edgeTension = 0.95;
...
Renderer.prototype.edgeInterpolate = function(value) {
  if (!arguments.length) { return this._edgeInterpolate; }
  this._edgeInterpolate = value;
  return this;
};

Renderer.prototype.edgeTension = function(value) {
  if (!arguments.length) { return this._edgeTension; }
  this._edgeTension = value;
  return this;
};
...
Renderer.prototype.run = function(graph, svg) {
  ...
  var svgNodes = createNodeRoots(graph, svg.select('g .nodes'));
  var svgEdgeLabels = createEdgeLabelRoots(graph, svg.select('g .edgeLabels'));
  var svgEdgePaths = createEdgePathRoots(graph, svg.select('g .edgePaths'));

  drawNodes(graph, this._drawNode, svgNodes);
  drawEdgeLabels(graph, this._drawEdgeLabel, svgEdgeLabels);
  ...
  drawEdges(result, this._drawEdge, svgEdgePaths, this._edgeInterpolate, this._edgeTension);
...
function createNodeRoots(graph, root) {
  return root
    .selectAll('g .node')
...
function createEdgeLabelRoots(graph, root) {
  return root
...
function createEdgePathRoots(graph, root) {
  return root
...
function drawEdges(graph, drawEdge, roots, edgeInterpolate, edgeTension) {
  roots.each(function(e) { drawEdge(graph, e, d3.select(this), edgeInterpolate, edgeTension); });
}
...
function defaultDrawEdge(graph, e, root, edgeInterpolate, edgeTension) {
  root
    .append('path')
