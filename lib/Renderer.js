...
function Renderer() {
  ...
  this._positionNodes = defaultPositionNodes;
  this._positionEdgeLabels = defaultPositionEdgeLabels;
  this._positionEdgePaths = defaultPositionEdgePaths;
...
Renderer.prototype.positionNodes = function(positionNodes) {
  ...
  this._positionNodes = positionNodes;
  return this;
};

Renderer.prototype.positionEdgeLabels = function(positionEdgeLabels) {
  if (!arguments.length) { return this._positionEdgePathsLabels; }
  this._positionEdgePathsLabels = positionEdgeLabels;
  return this;
};

Renderer.prototype.positionEdgePaths = function(positionEdgePathss) {
  if (!arguments.length) { return this._positionEdgePaths; }
  this._positionEdgePaths = positionEdgePaths;
  return this;
};
...
function defaultDrawNodes(g, root) {
  ...
  svgNodes.each(function(u) { addLabel(g.node(u).label, d3.select(this), 10, 10); });
...
function defaultDrawEdgeLabels(g, root) {
  ...
  svgEdgeLabels.each(function(e) { addLabel(g.edge(e).label, d3.select(this), 0, 0); });
...
function defaultPositionEdgePaths(g, svgEdgePaths) {
  svgEdgePaths.selectAll('path').property('points',
    function(e) {
      var value = g.edge(e);
  ...
  svgEdgePaths.filter('.enter')
    .selectAll('path')
      .attr('d', function() { return d3.select(this).property('points'); })
      .transition()
        .duration(1000)
        .style('opacity', 1);

  svgEdgePaths.filter(':not(.enter)')
    .selectAll('path')
      .transition()
        .duration(1000)
        .attr('d', function() { return d3.select(this).property('points'); });
