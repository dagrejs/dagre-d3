...
function createNodes(selection, g, shapes) {
  ...
  svgNodes.each(function(v) {
    ...
    if (node.subnodes && node.subnodes.length) {
      ...
      if (hoveredSubnode) {
        // The tooltip gets rendered into a group with class `qc-tooltip`.
        var tooltipGroup = thisGroup.append("g").attr("class", "qc-tooltip");

        // Append the tooltip text and measure its dimensions so we can size the surrounding
        // rectangle.
        tooltipGroup.append('text')
          .text(hoveredSubnode.ref['@type'][0])
          .attr('x', 0)
          .attr('y', 30);
        var labelBBox = tooltipGroup.node().getBBox();
        ...
        var labelBBox = tooltipGroup.node().getBBox();
        tooltipGroup.attr('transform', `translate(${-labelBBox.width / 2},0)`);
      }
    } else {
